...
client.on('ready', function() {
  ...
  setInterval(async () => {
    const statuslist = [
      `Stromy SMP Bots`,
      `With ${client.guilds.size} Server`,
      `With ${client.channels.size} Channel`,
      `With ${client.users.size} Friend`,
      `Prefix [ >help ]`,
    ...
    try {
      await client.user.setPresence({
        game: {
          ...
          type: "STREAMING",
          url: 'https://www.twitch.tv/stromycloud'
        },
        status: "dnd"
      });
    } catch (error) {
...
client.on("message", async message => {
  ...
  if(message.content.toLowerCase() === `<@${client.user.id}>`) {
    ...
    .setDescription(`:hand_splayed: Hi ${message.member} My name is Fukigen and my prefix in this server\nis ${prefix}`)
...
function play(guild, song) {
  ...
  .setImage(`https://i.ytimg.com/vi/FPBhvEfFhdU/maxresdefault.jpg`)
  .setFooter(`If this bot does not sound, you have to exit and log back in.`)
  .setTimestamp();
